/*
Main file for part 2b of the Project #2
Author: Marco Valero
File Title: test_tree.cc
...
In addition, it searches for each of the sequences in the file "sequences.txt" and removes every other sequence from the tree. Functions implemeted in "avl_tree.h" by Me will be of use.
  ...
  while(std::getline(file_two, lines_of_sec_file)) {
    if (GoodLine(lines_of_sec_file)) {
      ...
      if (a_tree.find(lines_of_sec_file, calls))
        querries_found++;
      total_calls += calls;
      calls = 0; // reset because find() modifies the number with every call
    }
  }
